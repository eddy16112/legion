...
//--------------------------------------------------------------------------
{
  switch(message.kind) {
    case TASK_STEAL_REQUEST:
    {
      ...
      printf("I received a message from proc %llx, msg %s\n", message.sender.id, message.message);
      break;
    }
                                                  ...
                                                  TaskOptions& output)
{
  ...
  output.initial_proc = select_processor_by_id(kind, 0);
}
    ...
    case 2:
      {
        ...
        for (PointInRectIterator<2> pir(rect); pir(); pir++, idx++)
        {
          ...
          output.slices[idx] = TaskSlice(slice,
              select_processor_by_id(task.target_proc.kind(), 0),
              false/*recurse*/, true/*stealable*/);
        }
        break;
      }
  ...
  //--------------------------------------------------------------------------
{
  ...
  hostname[1023] = '\0';
  gethostname(hostname, 1023);
  printf("i am in my select tasks to map, this %p, ctx %p, ready_tasks size %d, map_index %d, output.map_tasks.size %d, host %s\n", 
        this, ctx, input.ready_tasks.size(),map_index, output.map_tasks.size(), hostname);
}
                                       ...
                                       MapTaskOutput&        output)
{
  ...
  printf("i am in map_task %p, this %p, ctx %p, target proc %llx, host %s\n", &task, this, ctx, task.target_proc.id, hostname);
  ...
  output.task_priority = 0;//default_generate_random_integer();
  
  Processor::Kind kind = task.target_proc.kind();
                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  printf("Initializing field %d for block %d..., task %p, current proc %llx, target proc %llx\n", 
          fid, point, task, task->current_proc.id, task->target_proc.id);
                ...
                Context ctx, Runtime *runtime)
{
  ...
  printf("Running daxpy computation with alpha %.8g for point %d..., task %p, current proc %llx, target proc %llx, node %s, thread %ld\n", 
          alpha, point, task, task->current_proc.id, task->target_proc.id, hostname, self);
                ...
                Context ctx, Runtime *runtime)
{
  ...
  printf("Checking results..., task %p, current proc %llx, target proc %llx\n", 
          task, task->current_proc.id, task->target_proc.id);
  Rect<1> rect = runtime->get_index_space_domain(ctx,
                  task->regions[0].region.get_index_space());
